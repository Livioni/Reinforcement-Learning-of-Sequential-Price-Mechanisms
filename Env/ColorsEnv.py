...
class ColorsEnv(gym.Env):
    ...
    def __init__(self):
        ## 初始化动作和状态的上下限
        self.items_num = 20     #假设item有n个
        self.agent_num = 30     #假设agent有m个
        self.delta = 0         #调整生成agent的valuation function的correlations
        #agent顺序 red yellow blue 
        #item 顺序 red yellow 
        ...
        for i in range(self.agent_num):
            if i <= 9:
                for j in range(self.items_num):
                    if j <= 9:
                        self.valuationF[i][j] = 1
                    else: 
                        self.valuationF[i][j] = 0
            if 10 <= i <= 19:

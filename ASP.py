...
log_dir = "runs/ASP_"+str(start_time)
...
################### checkpointing ###################
run_num_pretrained = 'SPMs20-5ASP'  #### change this to prevent overwriting weights in same env_name folder
...
class ActorCritic(nn.Module):
    ...
    def evaluate(self, state, action):
        ...
        ##################################聚合信息###################################
        action_logprobs = action_logprob1 
        dist_entropy = dist1_entropy 
...
class PPO:
    ...
    def select_action(self, state, price):

        with torch.no_grad():
            state = torch.FloatTensor(state).to(device)
            action, action_logprob = self.policy_old.act(state, price)
...
def train():
    ...
    while time_step <= max_training_timesteps:
        ...
        current_ep_reward = 0
        price = np.random.randint(0,1000,(1,5))/1000
        price = torch.tensor(price,dtype=torch.float32).squeeze(0)
        for t in range(1, max_ep_len + 1):

            # select action with policy
            action = ppo_agent.select_action(state,price)

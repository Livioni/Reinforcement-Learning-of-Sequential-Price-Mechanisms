...
writer = SummaryWriter(comment='Stackelberg Reward Record')
print("============================================================================================")
####### initialize environment hyperparameters ######
env_name = "Stackelberg-v0"  # 定义自己的环境名称
...
################### checkpointing ###################
run_num_pretrained = 'StacklebergSimple'  #### change this to prevent overwriting weights in same env_name folder
...
class PPO:
    ...
    def sub_select_action(self,state):
        with torch.no_grad():
            state = torch.FloatTensor(state).to(device)
            sub_action = self.policy_old.sub_act(state)
        return sub_action.detach() 
...
def train():
    ...
    while time_step <= max_training_timesteps:
        ...
        state = env.reset()
        episode_num = 1
        while episode_num < sub_episodes:
            sub_action = ppo_agent.sub_select_action(state)
            state, reward, done, _ = env.step(sub_action)
            if done:
                episode_num += 1
                state = env.reset()
        ...
        for t in range(1, max_ep_len + 1):
            ...
            if done:
                ...
                env.reset_weights()
                writer.add_scalar('info/PPO_SW', socialwelfare/2.5, global_step=i_episode)
                break
